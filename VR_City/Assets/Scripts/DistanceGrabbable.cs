    ...
    public class DistanceGrabbable : OVRGrabbable
    {
        ...
        void RefreshCrosshair()
        {
            ...
            if (m_materialColorField != null)
            {
                ...
                if (isGrabbed || !InRange || OVRInput.Get(OVRInput.RawButton.RIndexTrigger))
                {
                    //m_mpb.SetColor(m_materialColorField, m_crosshairManager.OutlineColorOutOfRange);
                    m_renderer.material.color = Color.white; // Changed Here
                }
                else if (Targeted)
                {
                    //m_mpb.SetColor(m_materialColorField, m_crosshairManager.OutlineColorHighlighted);
                    m_renderer.material.color = Color.blue; // Changed Here
                }
                else
                {
                ...
                if(!ChangeView.instance.Builderview)
                {
                    m_renderer.material.color = Color.white; // Changed Here
                }
                //m_renderer.SetPropertyBlock(m_mpb);
